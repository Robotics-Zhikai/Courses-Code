...
* write到内核缓冲区的数据不一定全部都发到对端了，实际上就是CS144中所讲的write到Bytestream，然后按照TCP协议去发。read的话就是read Bytestream，Bytestream是TCP协议交付数据存放的地点，可以把Bytestream 当成内核缓冲区（见[本笔记这里](#第二章-传输层tcpudp和sctp)）。

## 第四章 基本TCP套接字编程
* listen
	* ![图4-7](./image/图4-7.png)
	* 涉及到一个syn泛滥的攻击：攻击者发送大量的TCPSYN报文段，而不完成第三次握手的步骤，随着这种SYN报文段纷至沓来，服务器不断为这些半开连接分派资源，导致服务器的连接资源被消耗殆尽，通过伪造的SYN来装填未完成连接队列，使合法的SYN排不上队，导致针对合法客户的服务被拒绝。解决方案在计算机网络自顶向下有描述，如下图
		*  ![SYNfloodatack](./image/SYNfloodatack.png)
	* 如果某客户不发送ACK来完成第三次握手，那么通常在一分钟后，服务器将终止该半开连接并回收资源。
...
* 当一个进程向某个已收到RST的套接字执行写操作时，内核向该进程发送一个SIGPIPE信号。写一个已接收FIN的套接字不成问题(因为TCP连接是全双工的，收到FIN意味着对端到本端的连接终止，但是并不意味着本端到对端的连接终止)，但是写一个已接收RST的套接字则是一个错误.
...
* 异步I/O操作：不导致请求进程阻塞
* 在I/O模型中，同步和异步区分的是内核向应用程序通知的是何种I/O事件（是就绪事件还是完成事件），以及该由谁来完成I/O读写（是应用程序还是内核）。引自Linux高性能服务器编程。
...
## 第14章 高级I/O函数
* 套接字设置超时的三种方法：
	* 调用alarm，超时期满产生SIGALARM信号，需要设置信号处理函数
	* 在select中阻塞等待I/O
...
## 第16章 非阻塞式I/O
* 输入操作：
	* 输入操作（包括read、readv、recv、recvfrom、recvmsg），对于阻塞的套接字，套接字的接受缓存中若没有数据可读，则该进程将被投入睡眠，直到有一些数据到达；对于非阻塞的套接字，如果输入操作不能被满足（对于TCP套接字即至少有一个字节的数据可读，对于UDP套接字即有一个完整的数据报可读），相应调用将立即返回一个EWOULDBLOCK错误
* 输出操作
	* 对于阻塞式套接字，输出操作（包括write、writev、send、sendto和sendmsg共5个函数），如果发送缓冲区中没有空间，进程将被投入睡眠，直到有空间为止。而对于非阻塞套接字，如果发送缓冲区中没有空间，输出函数调用将立即返回有个EWOULDBLOCK错误，如果发送缓冲区中有一些空间，返回值将是内核能够复制到该缓冲区中的字节数（不足计数）。
* 接受外来连接
	* accept函数，对阻塞的套接字调用，尚无新的连接到达，调用进程将被投入睡眠；对非阻塞的套接字调用，并且如果尚无新的连接到达，accept调用将立即返回一个EWOUDBLOCK错误。
* 发起外出连接
	* connect函数。对阻塞的套接字，涉及到一个三次握手的过程，而且connect函数一直要等到客户收到对于自己的SYN的ACK为止才返回（意味着至少阻塞一个RTT）。如果对于一个非阻塞的TCP套接字调用connect，并且连接不能**立即建立**，那么连接的建立能照样发起（譬如送出TCP三路握手的第一个分组），不过会返回一个EINPROGRESS错误；同时需指出的是，在服务器和客户处于同一个主机的情况下，连接可以立即建立。 
* 要注意避免使用标准I/O，主要是考虑到缓冲区管理复杂的问题.同时要注意到，非阻塞式I/O的加入让函数的缓冲区管理显著的复杂化了。
* 下图表明自定义的缓冲区和套接字本身具有的缓冲区的关系
	* ![图16-8](./image/图16-8.png) 
	* 总体思路大概就是select阻塞后（在各fd非阻塞的前提下），不断判断套接字发送缓冲区是否就绪，套接字接收缓冲区是否就绪，客户缓冲区是否就绪，stdin或stdout是否就绪等事件，然后当事件就绪时取消阻塞去执行对应的read write逻辑，这样的话就非常高效，不会出现某一fd被阻塞而不能响应其他事件的情况。
* 阻塞式connect调用在TCP三路握手完成前被中断的话假设中断的connect调用不由内核自动重启，那么它将返回EINTR；不能再次调用connect等待未完成的连接继续完成，这样将导致返回EADDRINUSE错误
* 非阻塞connect
	* 完成一个阻塞connect需要花费一个RTT
	* 非阻塞connect可以把三路握手叠加在其他处理上；可以同时建立多个连接； 
	* 关于select和非阻塞connect有以下两个规则（需要一些机制去判断connect是否建立连接成功）：
		* 当连接成功建立时，描述符变为可写（此时能被select感知到）
		* 当连接建立遇到错误时，描述符变为即可读又可写（同样可被select感知到）
	* 非阻塞connect立即成功时，返回0
	* 非阻塞式connect可用于建立并发的TCP连接
* 多进程和多线程可以同样实现这里所示技术达成的效果
