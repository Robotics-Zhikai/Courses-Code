...
class ByteStream {
  private:
    ...
    size_t _beginIndex = 0; //是当前未读数据的初始位置
    size_t _length = 0; //当前数据流中最多能被读出的数据长度
  ...
  public:
    ...
    //! \returns `true` if the output has reached the ending //注意区分eof和input_ended，前者是读取流读取到了end，后者是流的末尾到了end，不一定读到了end
    bool eof() const;
    //!@}
