...
size_t ByteStream::write(const string &data) {
    if(input_ended()){ //如果已经到了eof了，就说明不能写了，再写就报错 或者直接丢掉数据，我也不知道该选哪个
        throw runtime_error("inputended");
    }
    size_t writedDataNum = min(data.size(),_cap-_length); //_cap-_length是剩余容量
