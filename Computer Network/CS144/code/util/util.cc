...
//! \note This class returns the checksum in host byte order. 返回主机字节顺序的校验和
...
void InternetChecksum::add(std::string_view data) { //整个add函数就是把data中的所有16位字都加起来，放入_sum中
    for (size_t i = 0; i < data.size(); i++) {
        uint16_t val = uint8_t(data[i]);
        if (not _parity) { //not等价于！
            val <<= 8; //每隔8位就进一次
        }
        _sum += val;
        _parity = !_parity;
...
uint16_t InternetChecksum::value() const {
    uint32_t ret = _sum;
    //当_sum全为1，即有32个位均为1时，则value返回0.
    //当_sum中有一个位为0，value返回非0，说明数据被损坏了
    //跟计算机网络自顶向下描述的UDP检验和原理是一样的，见上边给出的维基百科链接
    while (ret > 0xffff) { //这就是回卷操作的代码实现
